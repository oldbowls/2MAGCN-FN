...
class Single_SGN(nn.Module):
    def __init__(self, num_classes, num_joint, seg, regu1, regu2, bias=True, dim=256, adaptive_transform=False,
                 num_joint_ori=25,
                 gcn_type='mid'):
        ...
        self.spa_net = SpatialNet(num_joint, regu1, regu2, bias, dim, gcn_type)
...
if __name__ == '__main__':
    ...
    model = Single_SGN(num_class, num_j, num_t, adaptive_transform=True, gcn_type='small', num_joint_ori=num_j_o)
    # torch.save(model.state_dict(), '../../pretrain_models/single_sgn_jpt{}.state'.format(num_j),)
    dummy_data = torch.randn([1, 3, num_t, num_j_o, 1])
    # a = model(dummy_data)
    # a.mean().backward()
    # 18.81,0.67    ----------     15.75 0.67
    #
    hooks = {}
